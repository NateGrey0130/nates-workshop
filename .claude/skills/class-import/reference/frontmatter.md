...
"One additional attack" and "plus those gained from hand to hand" are bonuses
(`attacks:`), and a book that says the style adds no attacks takes the
top-level flag `ignores_style_attacks: true` beside its `attacks_base`. The
headers of `apps/character-creator/db/~003-class-attacks-are-bonuses.sql` and
`apps/character-creator/db/~004-ignores-style-attacks.sql` carry the readings
and their book pages.
