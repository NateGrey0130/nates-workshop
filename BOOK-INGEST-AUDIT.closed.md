**BOOK-INGEST-AUDIT.md, the closed findings - moved here 2026-09-16, text and numbering unchanged.**
Every block below is a finding whose own section records it as taken, closed, declined, adjusted,
withdrawn or shipped, moved verbatim from the live menu, which keeps a one-line pointer per finding.
Nothing here is open. This is a RECORD: do not rewrite a measurement (audit-menu). The live menu's
status header still governs; the live file holds every finding without an outcome lead of its own.
