...
// Joined on the slug since RETRO-AUDIT R21 - same arms as the character
// sheet's read, and the same reason: a gear id is insertion order. The legacy
// id arm went with migration 046, which dropped the column.
...
export async function onRequestPost({ request, env, params }) {
  ...
  // The client still sends a numeric `item_id` and the column that stored it is
  // gone, so the slug is derived from that id INSIDE the statement. One round
  // trip, and no window in which a caller could supply one key without the
  // other. RETRO-AUDIT R21.
