...
//
// A DOTTED ACRONYM COLLAPSES TO ONE TOKEN - `A.T.V.` to `atv`, `M.D.C.` to
// `mdc` - and that line is doing real work rather than tidying. Without it the
// separator strip below turns `A.T.V.` into the three tokens `a t v`, which can
// never match the single token `atv`, so the acronym that makes two names
// obviously the same machine becomes the thing that stops them scoring.
// Measured on production 2026-09-09 by calling `similarity` directly:
// `Big Boss ATV` / `The Big Boss A.T.V.` scored 0.500 and
// `Mountaineer ATV` / `The Mountaineer A.T.V.` scored 0.333, both under the 0.7
// threshold, so `findDuplicates` reported neither - and both are the same
// machine imported twice. `BOOK-INGEST-AUDIT.md` F44.
//
// TWO OR MORE letters, so a name ending in an initial (`A.`) or an ordinary
// abbreviation (`St. John`) is untouched; only a run reads as an acronym.
...
// One pair, one identity. The detector walks rows in id order, so which of a
// pair arrives as `a` depends on insertion order and differs between production
// and a rebuilt database — a dismissal keyed the way it happened to be shown
// would silently stop matching. Sorting is what makes the stored UNIQUE
// constraint mean "this pair", and it is why the columns are written sorted
// rather than sorted on read: a row a human can query has to be canonical too.
//
// Case-folded to agree with the columns' own COLLATE NOCASE.
// BOOK-INGEST-AUDIT F33.
export function pairKey(keyA, keyB) {
  const [x, y] = [String(keyA ?? '').toLowerCase(), String(keyB ?? '').toLowerCase()].sort();
  // A NUL SEPARATOR, written as an ESCAPE rather than a literal byte. NUL
  // cannot occur in a catalog name, which is why it is the separator - no name
  // can contain one and be mistaken for a pair.
  //
  // It was a raw 0x00 in the source until 2026-09-09, and that made this whole
  // FILE read as binary: `grep` skipped it with "binary file matches", and
  // `git diff` rendered every change to it as `Bin 24839 -> 25820 bytes` - so
  // the F44 normaliser change shipped with a diff no reviewer could read.
  // `\u0000` is the same string at runtime, byte for byte, and leaves the file
  // as text. `BOOK-INGEST-AUDIT.md` F44 named it; F48 changed it.
  return `${x}\u0000${y}`;
}

// Write the NO. There is deliberately no counterpart for YES: a confirmed
// duplicate goes through `mergeRows`, which deletes the losing row, so the pair
// cannot be suggested again and a row recording the merge would be a second
// copy of something the catalog already says.
export async function dismissPair(env, catalogKey, keyA, keyB, note, email) {
  const cat = CATALOGS[catalogKey];
  if (!cat) return { error: 'Unknown catalog', status: 400 };
  const a = String(keyA ?? '').trim(), b = String(keyB ?? '').trim();
  if (!a || !b) return { error: 'Both keys are required', status: 400 };
  if (a.toLowerCase() === b.toLowerCase()) {
    return { error: 'A row is not a pair with itself', status: 400 };
  }

  // Sorted on the way IN, so the stored row is canonical and the UNIQUE
  // constraint does the de-duplicating rather than the reader.
  const [lo, hi] = [a, b].sort((x, y) => (x.toLowerCase() < y.toLowerCase() ? -1 : 1));
  await env.DB.prepare(
    `INSERT OR IGNORE INTO catalog_pair_dismissals (catalog, key_a, key_b, note, dismissed_by)
     VALUES (?, ?, ?, ?, ?)`
  ).bind(catalogKey, lo, hi, note ? String(note).slice(0, 500) : null, email).run();
  return { ok: true, catalog: catalogKey, key_a: lo, key_b: hi };
}

// Undo one. A dismissal is a judgement and judgements are revisable — without
// this the only way back is the database, and a one-way button is one nobody
// presses.
export async function restorePair(env, catalogKey, keyA, keyB) {
  if (!CATALOGS[catalogKey]) return { error: 'Unknown catalog', status: 400 };
  const [lo, hi] = [String(keyA ?? '').trim(), String(keyB ?? '').trim()]
    .sort((x, y) => (x.toLowerCase() < y.toLowerCase() ? -1 : 1));
  const res = await env.DB.prepare(
    'DELETE FROM catalog_pair_dismissals WHERE catalog = ? AND key_a = ? AND key_b = ?'
  ).bind(catalogKey, lo, hi).run();
...
export async function findDuplicates(env, catalogKey) {
  ...
  // Pairs a human has already judged DISTINCT. Filtered here rather than in the
  // caller so every reader gets the same answer - the panel, the badge the
  // catalog page fires on every load, and the tests.
  //
  // The same argument the NPC sweep makes about its own dismissals: a
  // suggestion somebody already rejected is worse than noise, because it is the
  // button asking the same question again. Gear suggests 591 pairs, 589 of them
  // in the loosest tier, so without this the 45 pairs INGESTION-AUDIT F29 read
  // and judged on 2026-09-06 sit in that list indistinguishable from pairs
  // nobody has ever looked at. BOOK-INGEST-AUDIT F33.
  const dismissed = new Set();
  try {
    const { results: rows } = await env.DB.prepare(
      'SELECT key_a, key_b FROM catalog_pair_dismissals WHERE catalog = ?'
    ).bind(catalogKey).all();
    for (const r of rows) dismissed.add(pairKey(r.key_a, r.key_b));
  } catch {
    // A database that has not had migration 050 applied still answers this
    // endpoint. Suggesting a pair somebody dismissed is the OLD behaviour and
    // is survivable; a 500 that hides every suggestion is not.
  }
  ...
  for (const idxs of buckets.values()) {
    for (let x = 0; x < idxs.length; x++) {
      ...
      if (dismissed.has(pairKey(a[cat.uniqueField], b[cat.uniqueField]))) continue;
