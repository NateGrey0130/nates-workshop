// The bits every app's test suite needs: how to record a result, how a
// section announces itself, and the summary line a merge is gated on.
//
// Shared since the codebase split into groups (groups.json): the tools and
// Marvel suites used to import this from apps/character-creator/test/, so a
// change to the character creator's test tree could break every other app's
// suite. The character creator's own harness.mjs re-exports all of this and
// adds the one thing only that app needs - where its moved pages live.
//
