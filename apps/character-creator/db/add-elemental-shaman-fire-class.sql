-- BOOK-INGEST-AUDIT F63, taken as an import: the Elemental Shaman (Fire),
-- Rifts World Book 15: Spirit West p.65-67.
--
-- One-off data script, run once per environment. NOT a migration.
--
--   node scripts/d1-apply.mjs --local apps/character-creator/db/add-elemental-shaman-fire-class.sql
--
...
SELECT 'elemental-shaman-fire', 'Elemental Shaman (Fire)', 'rifts', '---
id: elemental-shaman-fire
name: Elemental Shaman (Fire)
system: rifts
...
magic:
  ...
  spells_starting_groups:
    - { count: 3, from: ["Fire: Blinding Flash", "Fire: Cloud of Smoke", "Fire: Create Coal", "Fire: Fiery Touch", "Fire: Fire Bolt", "Fire: Globe of Daylight", "Fire: Impervious to Fire", "Fire: Nightvision", "Fire: Stench of Hades"], note: "Three first-level Fire spells, from the Warlock sphere the book points at (printed 65)." }
...
WHERE NOT EXISTS (SELECT 1 FROM imported_classes WHERE class_id = 'elemental-shaman-fire');

SELECT class_id, name, status, length(markdown) AS bytes, instr(markdown, char(13)) > 0 AS has_cr
  FROM imported_classes WHERE class_id = 'elemental-shaman-fire';

-- Records this run. See db/migrations/024-data-script-runs.sql.
INSERT INTO data_script_runs (filename) VALUES ('add-elemental-shaman-fire-class.sql');
