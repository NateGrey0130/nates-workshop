-- The Juicer O.C.C., updated to Rifts Ultimate Edition p.79-81.
--
-- The stored entry was audited against the ORIGINAL Rifts core book (its
-- extraction notes cite Rifts p.69-71); RUE revised the class, and the rest
-- of the catalog (spells, psionics, skills) is RUE. The full delta is in the
-- markdown's extraction_notes. Guarded on the old-edition marker text, so
-- re-running or running where the update has landed does nothing.
--
--   node scripts/d1-apply.mjs --local apps/character-creator/db/fix-juicer-rue-edition.sql
...
WHERE class_id = 'juicer'
  AND instr(markdown, 'AUDIT (Rifts p.69-71)') > 0;

-- Read the result back rather than trusting the exit code.
SELECT class_id, length(markdown) AS bytes, instr(markdown, 'Rifts Ultimate Edition p.79-81') > 0 AS is_rue,
       instr(markdown, char(13)) > 0 AS has_cr FROM imported_classes WHERE class_id = 'juicer';

-- Records this run. One row per run rather than per file: every statement
-- above guards itself, so this script is safe to re-run and safe to run
-- early, and a run that correctly did nothing is still a run that happened.
-- See db/migrations/024-data-script-runs.sql.
INSERT INTO data_script_runs (filename) VALUES ('fix-juicer-rue-edition.sql');
