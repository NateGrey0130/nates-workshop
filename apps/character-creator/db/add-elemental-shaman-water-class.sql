-- BOOK-INGEST-AUDIT F63, taken as an import: the Elemental Shaman (Water),
-- Rifts World Book 15: Spirit West p.65-67.
--
-- One-off data script, run once per environment. NOT a migration.
--
--   node scripts/d1-apply.mjs --local apps/character-creator/db/add-elemental-shaman-water-class.sql
--
...
SELECT 'elemental-shaman-water', 'Elemental Shaman (Water)', 'rifts', '---
id: elemental-shaman-water
name: Elemental Shaman (Water)
system: rifts
...
magic:
  ...
  spells_starting_groups:
    - { count: 3, from: ["Water: Cloud of Steam", "Water: Color Water", "Water: Create Fog", "Water: Dowsing", "Water: Float on Water", "Water: Purple Mist", "Water: Salt Water to Fresh", "Water: Sense Direction Underwater", "Water: Walk the Waves", "Water: Water to Wine"], note: "Three first-level Water spells, from the Warlock sphere the book points at (printed 65)." }
...
skills:
  occ_skills:
    ...
    - { name: "Swimming", base: 98, per_level: 0, note: "The Water Shaman''s element skill, at 98% (printed 66)." }
  occ_related_skills:
    count: 9
...
WHERE NOT EXISTS (SELECT 1 FROM imported_classes WHERE class_id = 'elemental-shaman-water');

SELECT class_id, name, status, length(markdown) AS bytes, instr(markdown, char(13)) > 0 AS has_cr
  FROM imported_classes WHERE class_id = 'elemental-shaman-water';

-- Records this run. See db/migrations/024-data-script-runs.sql.
INSERT INTO data_script_runs (filename) VALUES ('add-elemental-shaman-water-class.sql');
