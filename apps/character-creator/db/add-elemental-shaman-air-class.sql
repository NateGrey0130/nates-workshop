-- BOOK-INGEST-AUDIT F63, taken as an import: the Elemental Shaman (Air),
-- Rifts World Book 15: Spirit West p.65-67.
--
-- One-off data script, run once per environment. NOT a migration.
--
--   node scripts/d1-apply.mjs --local apps/character-creator/db/add-elemental-shaman-air-class.sql
--
...
--
-- THE BASE of the four: earth, fire and water are declared copies of this one.
-- Generated; apostrophes doubled; the markdown is pure ASCII with LF endings.

INSERT INTO imported_classes (class_id, name, system, markdown, status, created_by)
SELECT 'elemental-shaman-air', 'Elemental Shaman (Air)', 'rifts', '---
id: elemental-shaman-air
name: Elemental Shaman (Air)
system: rifts
...
magic:
  ...
  spells_starting_groups:
    - { count: 3, from: ["Air: Breathe Without Air", "Air: Cloud of Slumber", "Air: Cloud of Steam", "Air: Create Light", "Air: Create Mild Wind", "Air: Stop Wind", "Air: Thunderclap"], note: "Three first-level Air spells, from the Warlock sphere the book points at (printed 65)." }
...
skills:
  occ_skills:
    ...
    - { name: "Astronomy", base: 98, per_level: 0, note: "The Air Shaman''s element skill, at 98% (printed 65)." }
  occ_related_skills:
    count: 9
...
WHERE NOT EXISTS (SELECT 1 FROM imported_classes WHERE class_id = 'elemental-shaman-air');

SELECT class_id, name, status, length(markdown) AS bytes, instr(markdown, char(13)) > 0 AS has_cr
  FROM imported_classes WHERE class_id = 'elemental-shaman-air';

-- Records this run. See db/migrations/024-data-script-runs.sql.
INSERT INTO data_script_runs (filename) VALUES ('add-elemental-shaman-air-class.sql');
