-- The Chiang-Ku Dragon R.C.C., recovered into the repo.
--
-- One-off data script, run once per environment. NOT a migration - it adds
-- rows, not schema.
--
--   node scripts/d1-apply.mjs --local  apps/character-creator/db/add-chiang-ku-dragon-class.sql
--   node scripts/d1-apply.mjs --remote apps/character-creator/db/add-chiang-ku-dragon-class.sql
--
...
SELECT 'chiang-ku-dragon', 'Chiang-Ku Dragon', 'palladium-fantasy', '---
id: chiang-ku-dragon
...
variants:
  - id: hatchling
    ...
    # p.23: a hatchling starts with the LANGUAGE and BASIC MATH skills at 96%.
    # Domestic skills, advanced math and everything else start at first-level
    # proficiency, which is the catalog base.
    skill_overrides:
      - { name: "Advanced Math", base: 45 }
      - { name: "Cook", base: 25 }
      - { name: "Dance", base: 25 }
      - { name: "Fishing", base: 25 }
      - { name: "Play Musical Instrument", base: 25 }
      - { name: "Sewing", base: 25 }
    attribute_dice:
      IQ: "3d6+4"
      ME: "3d6+4"
...
The percentile chance of a young Chiang-Ku''s chosen field of study (50% Wizardry, 20% Diabolism, 20% Summoning, 5% other) can be used to quickly generate NPC dragons'' capabilities and outlook.', 'published', 'data-script'
WHERE NOT EXISTS (SELECT 1 FROM imported_classes WHERE class_id = 'chiang-ku-dragon');

-- Read the result back rather than trusting the exit code.
SELECT class_id, name, status, length(markdown) AS bytes, instr(markdown, char(13)) > 0 AS has_cr
  FROM imported_classes WHERE class_id = 'chiang-ku-dragon';

-- Records this run. REQUIRED: the smoke test fails a data script that has no
-- footer, or whose footer names a different file.
INSERT INTO data_script_runs (filename) VALUES ('add-chiang-ku-dragon-class.sql');
