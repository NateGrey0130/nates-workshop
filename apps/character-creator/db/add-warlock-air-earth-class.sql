-- RETRO-AUDIT R3, taken as an import: the Warlock (Air and Earth).
--
-- One-off data script, run once per environment. NOT a migration.
--
--   node scripts/d1-apply.mjs --local  apps/character-creator/db/add-warlock-air-earth-class.sql
--   node scripts/d1-apply.mjs --remote apps/character-creator/db/add-warlock-air-earth-class.sql
--
...
SELECT 'warlock-air-earth', 'Warlock (Air and Earth)', 'rifts', '---
id: warlock-air-earth
occ_group: magic
name: Warlock
system: rifts
...
', 'published', 'retro-audit-r3'
 WHERE NOT EXISTS (SELECT 1 FROM imported_classes WHERE class_id = 'warlock-air-earth');

SELECT 'warlock-air-earth is published' AS assertion, count(*) AS got, 1 AS want
  FROM imported_classes WHERE class_id = 'warlock-air-earth' AND status = 'published' AND deleted_at IS NULL;

INSERT INTO data_script_runs (filename) VALUES ('add-warlock-air-earth-class.sql');
