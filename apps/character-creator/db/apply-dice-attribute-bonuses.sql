-- Attribute bonuses the books state as dice, now that `bonuses.attributes`
-- accepts a roll and `bonuses.attribute_minimums` a floor.
--
--   Cyber-Knight (Rifts p.63): "Add +1D4 to M.A., M.E., P.S., P.E., and Spd."
--
--   Juicer (Rifts p.69): +2D6 to P.S. with a minimum of 22, +2D6 to P.E.,
--   +2D4x10 to Spd, and +2D4 to P.P. with a minimum of 20. The Spd bonus alone
--   is 20 to 80 points, and none of it was reaching the character.
--
-- Guarded with instr() on the bonus being absent, so re-running is a no-op.
-- (`_` is a wildcard in LIKE, which is why these do not use it.)
...
',
 ...
 WHERE class_id = 'cyber-knight'
   AND instr(markdown, 'attributes: { MA:') = 0;
...
INSERT INTO data_script_runs (filename) VALUES ('apply-dice-attribute-bonuses.sql');
