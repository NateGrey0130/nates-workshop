-- RETRO-AUDIT R3, taken as an import: the Warlock (Water).
--
-- One-off data script, run once per environment. NOT a migration.
--
--   node scripts/d1-apply.mjs --local  apps/character-creator/db/add-warlock-water-class.sql
--   node scripts/d1-apply.mjs --remote apps/character-creator/db/add-warlock-water-class.sql
--
...
SELECT 'warlock-water', 'Warlock (Water)', 'rifts', '---
id: warlock-water
occ_group: magic
name: Warlock
system: rifts
...
magic:
  ...
  spells_starting: 3
  spells_from: ["Water: Cloud of Steam", "Water: Color Water", "Water: Create Fog", "Water: Dowsing", "Water: Float on Water", "Water: Purple Mist", "Water: Salt Water to Fresh", "Water: Sense Direction Underwater", "Water: Walk the Waves", "Water: Water to Wine"]
...
', 'published', 'retro-audit-r3'
 WHERE NOT EXISTS (SELECT 1 FROM imported_classes WHERE class_id = 'warlock-water');

SELECT 'warlock-water is published' AS assertion, count(*) AS got, 1 AS want
  FROM imported_classes WHERE class_id = 'warlock-water' AND status = 'published' AND deleted_at IS NULL;

INSERT INTO data_script_runs (filename) VALUES ('add-warlock-water-class.sql');
