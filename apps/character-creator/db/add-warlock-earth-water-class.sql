-- RETRO-AUDIT R3, taken as an import: the Warlock (Earth and Water).
--
-- One-off data script, run once per environment. NOT a migration.
--
--   node scripts/d1-apply.mjs --local  apps/character-creator/db/add-warlock-earth-water-class.sql
--   node scripts/d1-apply.mjs --remote apps/character-creator/db/add-warlock-earth-water-class.sql
--
...
SELECT 'warlock-earth-water', 'Warlock (Earth and Water)', 'rifts', '---
id: warlock-earth-water
occ_group: magic
name: Warlock
system: rifts
...
', 'published', 'retro-audit-r3'
 WHERE NOT EXISTS (SELECT 1 FROM imported_classes WHERE class_id = 'warlock-earth-water');

SELECT 'warlock-earth-water is published' AS assertion, count(*) AS got, 1 AS want
  FROM imported_classes WHERE class_id = 'warlock-earth-water' AND status = 'published' AND deleted_at IS NULL;

INSERT INTO data_script_runs (filename) VALUES ('add-warlock-earth-water-class.sql');
