...
--
--   node scripts/d1-apply.mjs --local apps/character-creator/db/~006-rue-xp-ladders.sql
--
...
--
-- RENAMED TWICE AFTER IT WAS FIRST APPLIED. It reached production as
-- zzzzzzzzzzzzzzzzzz-rue-xp-ladders.sql - an eighteenth z, a tier the
-- ordering table in docs/operations.md retires in favour of ~NNN-. It was then
-- ~004-rue-xp-ladders.sql, until #1413 put ~004-ignores-style-attacks.sql
-- beside it and two files shared the ~004 prefix. The two write different
-- classes, so ~006 (the next free number) moves nothing but the name. It
-- repoints the South America notes that named either old file, and deletes
-- the old run records, which name files the repo does not have (the
...
UPDATE imported_classes SET markdown = replace(markdown, 'xp_table, so there is nothing to copy. Checked 2026-09-25.', 'xp_table at import; ~006-rue-xp-ladders.sql then gave both' || char(10) || '    RUE''s City Rat & Vagabond ladder (RUE printed 295).'), updated_at = datetime('now') WHERE class_id = 'grimbor' AND instr(markdown, 'xp_table, so there is nothing to copy. Checked 2026-09-25.') > 0;
UPDATE imported_classes SET markdown = replace(markdown, 'xp_table in production (checked --remote 2026-09-25), so none is stored.', 'xp_table at import; ~006-rue-xp-ladders.sql then gave both' || char(10) || '    RUE''s Mind Melter, Ley Line Walker & Ley Line Rifter ladder (printed 295).'), updated_at = datetime('now') WHERE class_id = 'shaydor-spherian' AND instr(markdown, 'xp_table in production (checked --remote 2026-09-25), so none is stored.') > 0;
UPDATE imported_classes SET markdown = replace(markdown, 'chiang-ku-dragon, dragon-ray) stores an xp_table, so none is stored here.', 'chiang-ku-dragon, dragon-ray) stored an xp_table at import;' || char(10) || '    ~006-rue-xp-ladders.sql then gave the hatchlings and this' || char(10) || '    class RUE''s Dragon Hatchling & Adult Dragon ladder (RUE printed 295).'), updated_at = datetime('now') WHERE class_id = 'pogtalian-dragon-slayer' AND instr(markdown, 'chiang-ku-dragon, dragon-ray) stores an xp_table, so none is stored here.') > 0;
UPDATE imported_classes SET markdown = replace(markdown, 'psi-stalker in production stores no xp_table, so none is stored here.', 'psi-stalker stored no xp_table at import; ~006-rue-xp-ladders.sql then gave both RUE''s Burster, Psi-Stalker & Mystic ladder (RUE printed 295).'), updated_at = datetime('now') WHERE class_id = 'werejaguar' AND instr(markdown, 'psi-stalker in production stores no xp_table, so none is stored here.') > 0;

-- Where an earlier run already wrote those notes, they name an old file.
UPDATE imported_classes SET markdown = replace(markdown, 'zzzzzzzzzzzzzzzzzz-rue-xp-ladders.sql', '~006-rue-xp-ladders.sql'), updated_at = datetime('now') WHERE class_id IN ('tribal-shaman', 'werejaguar', 'grimbor', 'shaydor-spherian', 'pogtalian-dragon-slayer') AND instr(markdown, 'zzzzzzzzzzzzzzzzzz-rue-xp-ladders.sql') > 0;
UPDATE imported_classes SET markdown = replace(markdown, '~004-rue-xp-ladders.sql', '~006-rue-xp-ladders.sql'), updated_at = datetime('now') WHERE class_id IN ('tribal-shaman', 'werejaguar', 'grimbor', 'shaydor-spherian', 'pogtalian-dragon-slayer') AND instr(markdown, '~004-rue-xp-ladders.sql') > 0;
...
SELECT 'and no note names an old file' AS assertion, count(*) AS got, 0 AS want
  FROM imported_classes WHERE instr(markdown, 'zzzzzzzzzzzzzzzzzz-rue-xp-ladders.sql') > 0 OR instr(markdown, '~004-rue-xp-ladders.sql') > 0;

DELETE FROM data_script_runs WHERE filename IN ('zzzzzzzzzzzzzzzzzz-rue-xp-ladders.sql', '~004-rue-xp-ladders.sql');
INSERT INTO data_script_runs (filename) VALUES ('~006-rue-xp-ladders.sql');
