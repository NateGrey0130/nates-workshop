...
>
> **All three remarks that once sat unnumbered inside closed findings were
> given numbers on 2026-09-12**, under `## Opened by the F2 remainder,
> 2026-09-12` in this file: `F2`’s reconciliation of its behaviours (1) and
> (2) is `F54`, `F6`’s `--border` remainder is `F55`, and `F17`’s print-ink
> observation is `F56`. **Read under each heading for its state** — this line
> gives none.
>
> **This clause was wrong twice on 2026-09-21, and the second time was a
> correction of the first.** It said THREE remarks were unnumbered; that was
> corrected the same morning to TWO, naming `F2`’s and `F6`’s; and TWO was
> wrong for exactly the reason THREE had been, because `F54` and `F55` had
> numbered those two on 2026-09-12 alongside `F56`. **The correction repaired
> one third of the error it was written to repair.** The re-read it recorded
> was of `UI-AUDIT.closed.md`, where `F2` and `F6` themselves live; the
> numbering that falsified it lives in a `##` section of THIS file. Kept
> rather than quietly replaced, because a correction reproducing the failure
> it corrects is the sharpest thing this header has to say.
>
