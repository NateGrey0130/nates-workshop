...
| `character_items` | Inventory join. `item_id` NULL means a freeform item (`custom_name` required). `gear_slug` is the **portable** key and the one reads join on - a gear id is insertion order and means nothing in another database (migration 044); `item_id` is kept beside it and the two are written together in one statement. `removed_at` NULL means currently held — removals are soft, so history survives. `enchantments` is a JSON array of `enchantments.slug`, on the **instance** rather than the catalog row. |
...
| `campaign_items` | The party stash: `character_items`' shape - including `gear_slug` beside `item_id` - owned by a campaign. `removed_at` NULL means still held; `claimed_by_character_id` says it left for a sheet rather than being spent or lost. |
