...
| `character_items` | Inventory join. `gear_slug` NULL means a freeform item (`custom_name` required). The slug is the **portable** key and, since migration 046, the only one: a gear id is insertion order and means nothing in another database, where a slug means the same thing everywhere. The wizard still POSTs a numeric id and the slug is derived from it inside the insert. `removed_at` NULL means currently held — removals are soft, so history survives. `enchantments` is a JSON array of `enchantments.slug`, on the **instance** rather than the catalog row. |
