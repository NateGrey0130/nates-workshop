...
Fifty-five tables in one shared D1 database (`nates-workshop-media`, bound as `DB`),
...
to FilamentForge and the two prefixed `msh_` belong to Marvel Heroes, each
...
| Table | Notes |
|---|---|
| `campaigns` | Top-level container. `gm_email` owns it. `gm_notes` is GM-only and stripped from non-GM API responses. `rest_rates` (JSON, per-hour recovery by pool, or NULL) is the table's own rest rates, set by the GM and preferred by the sheet over one device's (migration 054, UI-AUDIT F52) — no default numbers ship. `open` (default 1) is the join gate: 0 means only the GM and existing members may create a character in it — see [Membership is not a table](docs/campaign-and-play.md#membership-is-not-a-table). |
| `characters` | See below — several JSON columns. `class_variant` names which `variants` entry the character is; NULL means the class as written. `occ_class_id` is the O.C.C. taken alongside an R.C.C.; NULL means none. `totem` is the `totems.slug` a class's `totem:` key had the player pick; NULL means none, and a class that picks none never stores one. `ppe_base_spent` (migration 065) is P.P.E. burned out of the base for good - a bought Talent, or a spell that takes it from the caster - and the EFFECTIVE P.P.E. maximum is `ppe_max - ppe_base_spent`: `ppe_max` stays what was rolled so the validator and the level-up and re-roll paths that rewrite it can neither refuse nor erase the spend. `BOOK-INGEST-AUDIT.md` F101. `kind` (migration 070) is `'pc'` or `'npc'` - an NPC is a character row the campaign's G.M. statted and owns. |
...
| `character_items` | Inventory join. `gear_slug` NULL means a freeform item (`custom_name` required). The slug is the **portable** key and, since migration 046, the only one: a gear id is insertion order and means nothing in another database, where a slug means the same thing everywhere. The wizard still POSTs a numeric id and the slug is derived from it inside the insert. `removed_at` NULL means currently held — removals are soft, so history survives. `enchantments` is a JSON array of `enchantments.slug`, on the **instance** rather than the catalog row. `ammo_current` is the rounds a carried weapon has left, NULL meaning full - the magazine size is the gear row's payload (migration 083). |
| `character_vehicles` | The vessel a character owns, and the damage it carries. Migration 052. **Not a column on `character_items`**: that table's `CHECK` and its `gear(slug)` foreign key are load-bearing, the sheet's inventory table renders every row that endpoint returns (so a robot would arrive as a line item with a quantity box), and a quantity box and an equipped tick are the wrong questions about a robot, where `mdc_current` is the right one. `vehicle_slug` NULL means a freeform vessel (`custom_name` required), the same shape inventory uses. **`mdc_current` is JSON keyed by LOCATION NAME** — the maxima are catalog data in `vehicle_locations` and the damage is per-instance, since two Glitter Boys take different hits to the same arm. A location absent from the object is undamaged, so a fresh vessel is `{}`; that also means correcting a location's M.D.C. in the catalog does not rewrite every character who owns one. `removed_at` NULL means still owned — removal is soft, as inventory's is. `nickname` is what the player calls it; the catalog name is the model. |
...
| `pending_power_picks` | The same for spells and psionic powers. `spell_levels` is the cap the granting level carried, copied for the same reason — a Ley Line Walker's level-4 pair stays capped at spell level 4 however the class is later re-imported. `spell_traditions` is the tradition allowance copied the same way (migration 055); NULL on an older row is unrestricted. |
...
| `campaign_items` | The party stash: `character_items`' shape - `gear_slug` and no id - owned by a campaign. `removed_at` NULL means still held; `claimed_by_character_id` says it left for a sheet rather than being spent or lost. |
...
| `gear` | Gear catalog. `slug` is what `equipment_starting[].item_id` references, and since migration 046 the only key inventory holds — `NOT NULL` since 047, because `UNIQUE` alone permits any number of NULLs. Carries a stat block — damage, is_mega_damage, range, payload, rate_of_fire, ar, **sdc**, mdc — null wherever it does not apply, so one table covers weapons, armour and general kit. `sdc` is what the object TAKES before it breaks; `damage` is what it deals, and "1D6 S.D.C." on a knife is the second. `source_book` may say [not book-verified](#a-row-can-say-where-it-came-from). Named `gear`, not `items`, to stay clear of MediaVault's `media_items`. `vehicle_slug` (migration 053) means **this row is really a vessel**, recorded in `vehicles` under that slug — the row stays here because class markdown cites gear by slug and `catalog_redirects` cannot forward a key out of its own catalog. NULL on almost every row, and no foreign key, so a pointer to a vessel a later book session has not imported yet is inert rather than broken. `BOOK-INGEST-AUDIT.md` F41. |
| `vehicles` | Power armour, robots, drones, borg models, combat vehicles and ships - everything `gear` has never been able to hold. `mdc_main_body` is the **main body only**; every other part is a `vehicle_locations` row, so a reader wanting a total must sum them rather than trust the column. **The column's NAME is not its unit** — `is_mega_damage` is (migration 062). 0 means the number in `mdc_main_body` *and* in that vehicle's `vehicle_locations` rows is **S.D.C.**, which is what Heroes Unlimited prints; one M.D.C. point absorbs a hundred S.D.C., so writing a Patton's 1000 S.D.C. into a field read as M.D.C. would say it survives what a Glitter Boy survives. It defaults to 1 because every row in the table when the flag landed was a Rifts vessel. `ar` arrives with it: an Armour Rating is a to-hit threshold the rules read, the same number `gear.ar` holds for body armour, and it stays NULL for M.D.C. vessels, which do not have one. `vehicle_class` is free text on purpose: books invent categories, and a `CHECK` would reject a book rather than record it. **It is normalised by convention rather than by constraint** — `power-armor`, `robot`, `drone`, `borg`, `vehicle`, `ship` or `other` — because the column is what a vessel list shows in its meta column, and a list reading `robot` on one row and `Multi-purpose sea vessel.` on the next reads as a defect. Ingestion writes what the book gives; `zzzzzz-vehicle-class-vocabulary.sql` mapped the 50 rows that had arrived that way and **prepended each book phrase to its own `description`**, all 50 having been checked absent from it first. Keep both halves when a new book lands: one of the seven here, the book's own words there. Migration 048, from `BOOK-INGEST-AUDIT.md` F3. |
...
| `creatures` | The species the books stat, one row each (migration 074) - the Feathered Death, the Grimbor. The book's DICE for a kind of creature, where `notable_npcs` holds one person's numbers: `attributes` is JSON of the sheet's keys to formulas (`"2D6"`, `"4D6 min 17"` for a floor the book prints, or `"N/A"` for an attribute the species lacks), and hp / sdc / mdc / ppe / isp are formulas that may name an attribute (`"PE+20"`, `"PEx10"`), in the one strict grammar of `js/creature-roll.js`. `combat`, `ar` and `horror_factor` are fixed numbers; `playable` marks a species the book offers as a player race. Attacks are `stat_attacks` rows with `owner_kind = 'creature'`. `campaigns/[id]/npcs/from-creature` rolls individuals from it. |
...
system: rifts                 # rifts | palladium-fantasy | nightbane, required
...
| `me/holdings` | GET | Which of the **caller's own** characters (`player_email` = caller, `kind = 'pc'`) hold which skills, powers, gear, vessels and classes, keyed as the codex keys its entries — the codex's *yours* marks and *Your characters with this* line. Takes no parameters, so nothing in a request can widen it |
...
| `campaigns/[id]/items/[itemId]` | PATCH / DELETE / POST | qty/notes; soft remove; **POST claims it onto a character's sheet in one batch** — or `qty` of a stack, leaving the rest and a history row for the part taken |
...
| `campaigns/[id]/names` | GET | **G.M. only.** `?theme=&kind=&gender=&shape=&count=&avoid=` - a list of names from one theme of `shared/js/namegen.js`, none of them already a dossier, character or NPC in this campaign and none of the repeated `avoid` names (the chips on screen). `count` is clamped to 1-12. Returns `{ names, exhausted, reason }`: a theme that runs out returns **fewer, with the reason** - never a repeat or a borrowed name. An unknown theme, or a kind or shape the theme does not have, is a 400 |
| `names` | GET | Any signed-in user. The campaign-free list, for the character wizard's 🎲: the same parameters as `campaigns/[id]/names` (`?theme=&kind=&gender=&shape=&count=&avoid=`), and `{ names, exhausted, reason }` back, but no campaign's names are left out - only the `avoid` chips. Refuses rather than pads |
| `names/themes` | GET | Any signed-in user. `?system=` - the themes that game can use (its own and the generic ones) with their game and culture tags, the kinds each names and the shapes its people take, plus the class-to-theme, game-to-theme and game-to-places-theme default maps. No word lists |
| `campaigns/[id]/npcs/generate` | POST | **G.M. only.** Roll statted NPCs: `{ class_id, class_variant?, occ_class_id?, occ_class_variant?, level?, count? (1-10), name?, name_theme?, name_gender?, name_shape? }`. `name_theme` gives each NPC its own name from that theme, chosen before anything is written - a batch the theme cannot name is a **422 `names_exhausted`** and no NPCs; `name` and `name_theme` together are a 400. Each is a `characters` row with `kind = 'npc'`, owned by the G.M. and invisible to everyone else, written by the same `createCharacter()` a player's character goes through - so it is validated against its class the same way. Dice and choices are `js/npc-generate.js`. Spells, psionics and Talents the class lets the NPC choose are **banked** for the sheet's picks panel; powers it grants are held. A class it cannot build legally is a **422 naming what stopped it** (`code`: `needs_occupation`, `second_form`, `pool_exhausted`, ...) - never a padded pick |
...
| `npc-library` | GET / POST | The caller's NPC library (migration 079), **theirs alone**: GET lists their entries as summaries (name, game, source, class, level - no sheet); POST `{ character_id }` keeps a snapshot of a statted NPC from a campaign they run (a player's character is a 400). The three rollers take `to_library: true` to roll straight in: the roll runs its usual path, is snapshotted, and leaves nothing in the campaign |
| `npc-library/[id]` | GET / PATCH / DELETE | One entry with its whole sheet; PATCH renames it or sets the G.M.'s note on it; DELETE leaves copies already pulled. **Owner only - a 404, not a 403, to anyone else**, another G.M. included |
| `npc-library/[id]/pull` | POST | `{ campaign_id, force? }` - an INDEPENDENT copy in a campaign the caller runs (`requireCampaign` G.M.): a new `kind = 'npc'` row with the entry's picks, grants, items and vehicles, its notes ending with where it came from. A different game is a **409 `system_mismatch`** until `force` |
| `campaigns/[id]/cities` | GET / POST | The City Creator's saved cities in a campaign (migration 080). GET: the G.M. lists every one, anyone else only those whose map is shown - summaries only, never a city's contents. POST `{ city }` (**G.M. only**) saves one as generated |
| `cities/[id]` | GET / PATCH / DELETE | **G.M. only; a 404 to anyone else**, whether or not the map is shown - the players' view is a separate, smaller thing the server builds. PATCH `{ city?, show_map?, reveal?: {entryId: bool}, public?: {entryId: text} }` re-saves the city, shows the map, reveals a pin, or sets what the players read about an entry |
| `cities/[id]/view` | GET | **the players' view**: the map, the revealed pins and the players' lines, built from an allowlist so nothing else in the city is sent. The G.M. always; anyone else only while the map is shown, and a 404 otherwise |
| `city-themes` | GET / POST | The caller's own saved City Creator themes (migration 084). GET: summaries, `?system=` for one game's. POST `{ pack, name?, adapted_from? }` saves one; the pack is checked by the engine's `validateSavedTheme` and a 400 says what is wrong |
| `city-themes/[id]` | GET / PATCH / DELETE | **Owner only; a 404 to anyone else.** GET: the whole pack. PATCH `{ name?, pack? }` renames it or replaces its pack - checked again, and it stays the same game |
...
| `characters/[id]/items/[itemId]/stash` | POST | give it (or `qty` of it) back to the character's campaign stash, in one guarded batch, logged as a `stash` play event; refuses an enchanted item, which the stash cannot hold |
| `characters/[id]/vehicles` | POST | Give this character a vessel (catalog slug or freeform). Migration 052. No quantity and no equipped flag — the wrong questions about a robot |
| `characters/[id]/vehicles/[vehicleId]` | PATCH / DELETE | Nickname, notes, and **M.D.C. by location**; soft remove. Damage is checked against THAT vessel's own `vehicle_locations` rows and a key naming no location is refused, because stored unchecked it renders as a damaged part that does not exist. A freeform vessel joins to no catalog row and so takes damage anywhere — the same concession a freeform item gets for enchantments. Values may go NEGATIVE: Palladium blows straight through zero, and clamping would quietly disagree with the table |
...
| `characters/[id]/ppe-burn` | POST | `{ name }` - burn a held spell's `ppe_permanent` out of the character's P.P.E. base. The SERVER rolls the catalog's dice, because `ppe_base_spent` is not player-editable, adds the result to it and clamps current P.P.E.; the request names only the spell. No play event and no undo: the book calls the burn permanent. `BOOK-INGEST-AUDIT.md` F101 |
...
`source_book` normally names a book and a page. Twenty-three gear rows instead say:
...
This count said *Forty* until that day and measured **28**. `F46` then found three more of them in Rifts Ultimate Edition - the C-18 laser pistol on printed 257, Cyber-Armor on 65 and the hatchet on 56 - so it is **24** now. `F47` then found the hand axe too, on printed 99, so it is **23** now - and **not one of those 23 carries a combat number any more.** The rule the estimate tier states explicitly now happens to hold for the web marker as well, by searching rather than by fiat.
