...
| 2026-09-25 | #TBD | `~004-rue-xp-ladders.sql` renamed `~006-rue-xp-ladders.sql`: #1413 landed `~004-ignores-style-attacks.sql` and two files shared the prefix. Re-applied `--remote` as a no-op on the ladders; it repoints the five South America notes and replaces the run record |
