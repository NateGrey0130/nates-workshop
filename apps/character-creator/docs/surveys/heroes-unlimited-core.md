...
`parseClassMarkdown` and over all three places an entry lives, `variants[]`
included:

- **Eleven of the thirty classes have no named skill entry at all** - the nine
  Power Categories without sub-types plus three of the Alien educations. There
  is nothing in them to backfill.
- **The thirteen education classes with named entries already state THIS
  BOOK's base.** For each one, `stored - book base` is a constant equal to that
  class's educational bonus, while `stored - catalog base` is scattered;
  `hu-edu-trade-school`'s `Computer Operation` at 80 is the book's 60 plus 20,
  not the catalog's 40 plus 40. The eleven `hu-edu-*` bodies say so themselves.
  Rewriting them from the catalog's arithmetic would have made 323 correct
  entries wrong.
- **`hu-hardware` is the only class that ever asserted the
  catalog-plus-bonus convention**, in its own extraction notes, and F83
  generalised from it to twenty-nine classes that never claimed it.
- **Ten further candidates are printed ABSOLUTES** - `Mathematics: Basic` at
  98, `Automobile` at 96 - so deriving `book + (stored - catalog)` would push
  them to 106-133%.

So what is actually left of it is **about seventeen entries in `hu-hardware`
and `hu-hunter`**, plus three `per_level` disagreements, and
`hu-hardware`'s `Weapon Systems` at 60/+5, which
`zzzzzzzz-fix-hu-weapon-systems-base.sql` deliberately left alone and says so
