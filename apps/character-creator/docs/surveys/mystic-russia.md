...
written into them. **Three of this file's own counts were wrong and are
corrected in place** — the gear pages, the vessel count and the bestiary page
count. The pattern in all three was the same: *pages* carrying a structural
marker were counted as *entries*.

This was the **last book of the 2026-08-28 batch** (`BOOK-INGEST-QUEUE.md`).
...
| 6 | OCR text repair, **43 spells** | **2026-09-18**, `zzzzzzzzzzzzzz-fix-spell-psionic-ocr-text.sql`. The import read each **level heading** ("Level Two") onto the last spell of the level before it, and on `Nature: Negate Spoiling Magic` the next spell's name as well; plus `1D4><10`. **The digit cipher leaked into eight spells** (`!D6xlO` for 1D6x10, `lOOx`), two of them in the `damage` column - after a 2026-09-12 sweep had found it in none, because this book was imported the day after. Found by the read-only sweep that followed the super-ability repair (#1153). `Spoiling: Curdle Milk` ends with no stop because the page does, and `Bone: Crawling Bones` ends on its own table; both are asserted rather than touched. **The spell NAMED `Living Fire: Rerun's Celestial Fire Bolt` is a text-layer misread - printed 116 reads Perun's** - and is renamed in its own PR. Applied `--remote` before the merge ([#1154](https://github.com/NateGrey0130/nates-workshop/pull/1154)) |
| 7 | the Perun rename | **2026-09-18**, `zzzzzzzzzzzzzzz-rename-perun-celestial-fire-bolt.sql`. `Living Fire: Rerun's Celestial Fire Bolt` -> **`Living Fire: Perun's Celestial Fire Bolt`**, the name printed 116 carries (the text layer reads the heading "Rerun's"). Nothing cited the old name - 0 classes, characters, grants or drafts on production - and a `catalog_redirects` rename row keeps it resolving anyway, as the eight earlier spell renames do. Applied `--remote` before the merge |
...
already exists**, not a definition of a new one. The book says so outright: *"He
is fundamentally the same as the North American version described in the Rifts
RPG, page 83. The only substantive difference is the selection of spells"* —
then replaces item 8, Initial Spell Knowledge, with two common Wizard spells
...
25 skill grants across the six.** All six printed prowl, climb, swim and
...
with itself**: the Spirit Wolf puts Prowl in its *R.C.C. Skills* line and the
other five put theirs under Natural Abilities, so reproducing the seam gives one
creature a sheet-visible Prowl and five identical creatures none.

They are now granted as catalog skills at the printed figures with
`per_level: 0`, and the `natural_abilities` entries keep every detail a skill
row cannot hold — the rappel figure, the breath-holding, the depth, the
animal-form bonuses. **Prowl is deliberately absent from the Spirit Wolf's
list**: it already granted it, and a duplicate is refused with HTTP 422
`duplicate_skill`.

**One interaction, and it is the correct one.** `Track & Trap Animals` is a
Wilderness skill and five of the six grant Wilderness choice groups, so granting
it removes it from those pools through `takenNames()` — a skill cannot be taken
twice, and it replaces a pick at the catalog's base of 20% with the creature's
printed 60-90%. `Prowl`, `Climbing` and `Swimming` are Physical and
`Tracking (people)` is Espionage; no creature here grants a choice group from
either, so those fifteen grants interact with nothing.
...
**THE BOOK HAS ITS OWN SPELL INDEX AT PRINTED 6-7, and batch 1 did not use it.**
It lists every spell as `Name (cost) . . . page`, grouped by tradition and level -
the authority table `book-survey` §2 says to find. Auditing the 146 imported
rows against it found **one real error**: `Glimpse of the Future` shipped under
its SUBTITLE, `A Wood & Water Divination`, because printed 132 sets the name
above the subtitle above `Range:` and the reader walks back to the NEAREST
line. Corrected in PR #1007. It also showed **four Nature spells the reader
could never see** - `Speed of the Snail`, `Summon Fog`, `Calm Storms`,
