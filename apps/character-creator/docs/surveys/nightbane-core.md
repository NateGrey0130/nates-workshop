...
4. **SHIPPED - #1135, #1136, #1138 and #1141: all 19 classes** (15 playable per D3, plus D2's four package O.C.C.s), the last four after D5's catalog (#1137), second body (#1139) and generator (#1140). **The classes**, one batch per section, cited to the entry's own pages.
...
| 2026-09-16 | [#1136](https://github.com/NateGrey0130/nates-workshop/pull/1136) | **Five races**: Doppleganger (`nb-doppleganger`, printed 158-160), Hunter (`nb-hunter`, 164-165), Ashmedai (`nb-ashmedai`, 165-166), Namtar (`nb-namtar`, 167-168), Snake Bird (`nb-snake-bird`, 169). Ladders in `extraction_notes`: regression holds that no R.C.C. carries `xp_table`, so a race played alone levels on the default table even though this book prints a ladder for each. Doppleganger in `CORE_SDC_BY_CLASS` at 3D6; 1 gear stub. Classes 321 -> 326. Applied `--remote` before the merge. |
| 2026-09-16 | [#1137](https://github.com/NateGrey0130/nates-workshop/pull/1137) | **D5, PR 1 of 4: the Morphus catalog.** Migration 068 `morphus_characteristics` (one row per table entry, plus an intro per table; `key` = `<table>: <name>`; `horror_factor` TEXT because 73 entries add dice) and its data, **173 rows** off printed 91-106, reconciled against the page (4 defects fixed). Nothing reads it yet. Migration and data applied `--remote` before the merge. |
| 2026-09-17 | [#1138](https://github.com/NateGrey0130/nates-workshop/pull/1138) | **Four more classes and a psionic power**: Secondary Vampire (`nb-secondary-vampire`, printed 181-182), Wild Vampire (`nb-wild-vampire`, 182-183), Wampyr (`nb-wampyr`, 188-189), Guardian (`nb-guardian`, 189-192) - the Guardian has one body and does not wait for D5 - and D7's Super-Hypnotic Suggestion (20 I.S.P., printed 185) in a file sorting before every class script so no stub is emitted. The Secondary Vampire prints no I.S.P. and gets none. Classes 326 -> 330, psionic powers 132 -> 133. Applied `--remote` before the merge. |
...
| 2026-09-17 | [#1144](https://github.com/NateGrey0130/nates-workshop/pull/1144) | **Follow-ups 2 of 5: the multi-characteristic rule read as 81%.** The generator (#1140) ignored 61%+ as printed (printed 92), which made Unnatural Limbs unreachable from a multi-roll; per Nate it now ignores only the multi-characteristic rows (81%+), the likely erratum. Code only. |
| 2026-09-17 | [#1145](https://github.com/NateGrey0130/nates-workshop/pull/1145) | **Follow-ups 3 of 5: the Secondary Vampire's I.S.P.** The book prints none (printed 181-182), so #1138 shipped powers with no pool. Per Nate it now has `isp_base: "2d6x10"`, between the Wild Vampire (1D6x10) and the Master (3D6x10), and its note calls it a house figure. `fix-nb-secondary-vampire-isp.sql`, applied `--remote` before the merge. |
| 2026-09-17 | [#1146](https://github.com/NateGrey0130/nates-workshop/pull/1146) | **Follow-ups 4 of 5: race ladders.** An R.C.C. may now carry the `xp_table` its book prints, used when the race is played alone; in a pairing the O.C.C.'s wins (`combineClasses`, `xp_table` out of the race-wins loop). Regression's "no R.C.C. carries one" invariant is replaced. `fix-nb-race-xp-ladders.sql` stores the nine Nightbane race ladders (overlapping bounds plus one) and corrects their notes; `nb-nightbane` still states none. Applied `--remote` before the merge. |
| 2026-09-17 | [#1147](https://github.com/NateGrey0130/nates-workshop/pull/1147) | **Follow-ups 5 of 5: play reaches the Morphus.** Damage, healing, rest, the offline queue, undo and the G.M. dashboard act on the ACTIVE form; a Morphus pool goes negative exactly as the Facade's does (`damageCascade` never clamps; only a typed value stops at 0). The events route takes `changes.second_form` and names the form in the log. Damage stays tracked separately per form. Code only. |
