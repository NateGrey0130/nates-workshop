...
| spells, psionics, enchantments | `system` | one of `rifts`, `palladium-fantasy`, `nightbane`, `both` |
...
**There are THREE systems as of 2026-09-12, and the last row above is why they
...
stop at two while the other three take `nightbane`. The same rule keeps the
wizard's system picker at two: `S.system` feeds the campaign POST, and
`campaigns.system` is a third CHECK. `BOOK-INGEST-AUDIT` F73 has the decision
and what it would take to lift it.
