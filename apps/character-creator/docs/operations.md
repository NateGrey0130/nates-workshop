...
| `040-media-vault-source-id.sql` | **In `db/migrations/tools/` since 2026-09-25**, with its table, which lives in the tools group's own database (`DB_TOOLS`). `media_items.source_id` — where a MediaVault row came from, so its lookup can be re-run exactly. Not this app's table either. One generic column rather than two nullable ones: the normalised ISBN for a book, `tmdb:movie:1234` / `tmdb:tv:1234` for video, so the prefix names the lookup and the rest is its key. Empty on every pre-existing row, which is why the backfill it enables matters more than the column |
...
| `080-cities.sql` | `cities` - the City Creator's saved cities, each in a campaign (Prompt 2 Phase 3, 2026-09-23). `data` holds the city AS GENERATED, map and AI name pool included, never just its seed: the tables and the layout will change and a saved city must not change with them. NO owner column - a city's G.M. is its campaign's `gm_email` (requireCampaign). Everything is the G.M.'s until shown: `show_map`, plus a `reveal` flag per pin and a separate `public` text per entry inside `data`, so a secret cannot leak through a shared field. Seed line directly after the table. |
| `081-msh-power-text.sql` | **In `db/migrations/marvel/` since 2026-09-25**, with its table, which lives in Marvel's own database (`DB_MARVEL`). `msh_power_text` - Marvel Heroes' full text of the Ultimate Powers Book's power listings, one row per power code plus one per section introduction (2026-09-23). ANOTHER APP'S TABLE, hence the `msh_` prefix; `apps/marvel-heroes/README.md` documents it. **Its rows are never in the repo**: the repo is public and the text is TSR's, so `scripts/msh-extract.py` writes the data script into the gitignored `.cache/msh/` and `d1-apply` loads it from there. A database built from the repo has the table EMPTY, and the app falls back to its committed summaries. Seed line directly after the table. |
| `082-msh-heroes.sql` | **In `db/migrations/marvel/` since 2026-09-25**, with its table, which lives in Marvel's own database (`DB_MARVEL`). `msh_heroes` - Marvel Heroes' saved heroes, one row per hero, every read and write scoped to the Access email that saved it (2026-09-23). ANOTHER APP'S TABLE, hence the `msh_` prefix; `apps/marvel-heroes/README.md` documents it. Holds the generator's build state, a snapshot of what it built, and the sheet's written fields, all as JSON. Seed line directly after the table. |
| `083-character-item-ammo.sql` | `character_items.ammo_current` - rounds left in a carried weapon, as a number (NULL = full; the magazine size is the gear row's payload). Replaces the `ammo N/M` marker play mode kept in the row's notes, which a replayed offline shot rewrote wholesale. No backfill: production held no such marker when it shipped (2026-09-24, 0 of 44 rows). Seed line after 035's, which is after the `character_items` CREATE its guard reads. |
| `084-city-themes.sql` | `city_themes` - a G.M.'s saved City Creator themes, for reuse (2026-09-24). A theme costs five AI calls to write; this keeps a good one. `pack` is the whole theme, name pool included, **checked by the engine's own `validateSavedTheme` on every write** - the same rules a freshly written theme meets. OWNER ONLY by `owner_email`, a 404 to anyone else, as `npc_library` is. A city made from one keeps its own copy of the pack, so editing or deleting a saved theme never changes a kept city. One game per pack (`system`); `adapted_from` points at the theme it was adapted from for another game, `ON DELETE SET NULL`. Seed line directly after the table. |
| `085-drop-moved-group-tables.sql` | Drops the ten tables that moved out of this database (2026-09-25): the six `ff_` and two `media_` tables, now in the tools' `DB_TOOLS`, and the two `msh_` tables, now in Marvel's `DB_MARVEL`. They had stayed here unread as a rollback; every row was compared against its new copy and all three databases backed up before this was applied. 039, 040, 051, 081 and 082 keep their rows in this database's `schema_migrations` as history. Seed line at the end of `schema.sql`, guarded on none of the ten existing. |
...
| `039-filament-forge.sql` | **In `db/migrations/tools/` since 2026-09-25**, with its tables, which live in the tools group's own database (`DB_TOOLS`). The six `ff_` tables — FilamentForge's whole server side, not this app's. Its OFD catalog snapshot (`ff_brands`, `ff_filaments`, refreshed by `scripts/ofd-refresh.mjs`) and what its localStorage used to hold (`ff_config`, `ff_history`, `ff_presets`, `ff_custom_filaments`), keyed to the Access email the way `media_items` is |
...
| After | Rows |
|---|---|
| catalog rows citing no surveyed book | 230 |
...
| Sorts after **every** `z-` tier above | `~*.sql`, named `~NNN-` | **The counter stops at seventeen `z`s, and this is where it goes instead.** `~` is U+007E and `z` is U+007A, so one `~` sorts after any number of z's — and `NNN` is a numeric counter with room left, which is what the z's were standing in for all along. It is the slot the eighteenth `z` would have taken, documented before it was needed. **The first file in it is `~001-men-of-arms-frontmatter.sql`** (2026-09-25), which moves the S.D.C. grouping into class markdown and has to run after every script that rewrites a class's markdown. `test/checks/environment.mjs` → *Data script conventions* enforces both halves — it fails a data script carrying an eighteenth `z` and names this row, and it pins the sort property itself so this paragraph cannot quietly stop being true. **Why `~` is safe and a locale cannot break it:** all four things that order these files use JavaScript's own `.sort()`, which is UTF-16 code-unit order and reads no locale — `scripts/rebuild-local.mjs`, `scripts/repo-vs-live.mjs`, `scripts/d1-apply.mjs` (which re-globs a directory itself rather than trusting the shell) and `test/regression.mjs` step `[1/7]`. A shell glob *would* be locale-sensitive; nothing here depends on one. The rows above still read as a counter and not a category, and so does this one: `~001-` means *later than everything above*, never severity |
