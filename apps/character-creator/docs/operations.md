...
| `006-import-sessions.sql` | `import_sessions` + `import_staged` — **both tables are now unused**; the in-app importer that wrote them was retired without ever having written a row in production. The migration stays because it has run; the tables are left in place rather than dropped, which is a decision on its own |
